...
describe HtmlReportConfig do
  ...
  describe '#method_missing' do
    ...
    context 'with board-specific charts across multiple boards' do
      let(:board) { project_config.all_boards[1] }
      let(:board2) do
        ...
        Board.new(raw: raw, possible_statuses: load_complete_sample_statuses).tap do |b|
          project_config.add_issues([MockIssue.empty(board: b, key: 'SP-99')])
        end
      end
      let(:two_board_issues) do
        [
          MockIssue.empty(board: board),
          MockIssue.empty(board: board2, key: 'SP-99')
        ]
      end
